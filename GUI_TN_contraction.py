...
class NodePropertiesDialog(QDialog):
    def __init__(self, node):
        ...
        if rank == 0:
            ...
            indices = np.ndindex(*dims)  # Fixed line
...
class MainWindow(QMainWindow):
    ...
    def perform_contraction(self, node1, node2, selected_edges):
        ...
        remaining_items = []
        for idx, leg in enumerate(node1.legs):
            remaining_items.append(('leg', leg))
        for idx, edge in enumerate(node1.edges):
            if edge not in selected_edges:
                remaining_items.append(('edge', edge))
        for idx, leg in enumerate(node2.legs):
            remaining_items.append(('leg', leg))
        for idx, edge in enumerate(node2.edges):
            if edge not in selected_edges:
                remaining_items.append(('edge', edge))
    ...
    def showHelp(self):
        ...
        <style>
            li {
                font-size: 3.6em; /* Adjust the value as needed */
            }
        </style>
...
"""
        ...
        help_dialog.setWindowTitle("How to Use")
        layout = QVBoxLayout()
        text_edit = QTextEdit()
